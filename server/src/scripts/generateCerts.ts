...
// Certificates should be in server/certs/ (not server/src/certs/)
// __dirname in ts-node is src/scripts/, so we go up two levels to server/, then into certs/
...
try {
  ...
  // Generate certificate signing request and self-signed certificate with SAN
  console.log('2. Generating self-signed certificate with Subject Alternative Names...');
  
  // Create a config file for OpenSSL with SAN
...
`;
  ...
  console.log('   ✅ Certificate generated with SAN\n');
